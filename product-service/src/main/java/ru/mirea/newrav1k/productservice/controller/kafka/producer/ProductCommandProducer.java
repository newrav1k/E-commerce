...
public class ProductCommandProducer {
    ...
    public void processSuccessReserved(UUID orderId, UUID customerId, BigDecimal total) {
        log.info("Sending product reserved event to topic {}", this.productReservedTopic);
        SagaProductReservationSuccessEvent event = new SagaProductReservationSuccessEvent(orderId, customerId, total);
